...
from mtrfg.config.config import default_cfg, ud_paths, conllu_paths
...
print(f'Current dataset format: {dataset_format}')
if dataset_format == 'conllu':
    default_cfg.update(conllu_paths)
elif dataset_format == 'ud':
    default_cfg.update(ud_paths)

dh = DataHandler(format=dataset_format)

if dataset_format == 'conllu' and (config['parser_step_embeddings_flag'] or config['bert_lhs_step_embeddings']):
    raise ValueError('Conllu data does not have step indices and thus cannot be used for step embeddings.')

## if label index map is provided, we must use that to override number of labels
label_index_map = load_json(args['labels_json_path']) if 'labels_json_path' in args else dh.get_label_index_mapping(config['train_file'])
...
## build a dataloader 
train_loader = build_dataloader(config = config, loader_type = 'train', collate_function = ner_collate_function, label_index_map = label_index_map, data_handler = dh)
# train_loader = unified_dataloader(config, loader_type = 'train', collate_function = ner_collate_function, label_index_map = label_index_map, data_handler = dh)

save_json(train_loader.dataset.label_index_map, os.path.join(config['save_dir'], 'labels.json'))

## build a validation dataloader
val_loader = build_dataloader(config = config, loader_type = 'val', collate_function = ner_collate_function,label_index_map = label_index_map, data_handler = dh)
...
model = build_model(config,
                    model_start_path = model_start_path,
                    verbose=True,
                    ) 
print(config)
...
with tqdm(range(config['epochs'])) as pbar:
    for epoch in pbar:
        ...
        val_results, _ = run_evaluation(model, val_loader, eval_function = eval_function, config = config, label_index_map = label_index_map, data_handler = dh)
...
val_results, benchmark_metrics = run_evaluation(model, val_loader, eval_function = evaluate_model, config = config, label_index_map = label_index_map, data_handler = dh)
...
test_loader = build_dataloader(config = config, loader_type = 'test', collate_function=ner_collate_function, label_index_map=label_index_map, data_handler = dh)
test_results, benchmark_metrics = run_evaluation(model, test_loader, eval_function = evaluate_model, config = config, label_index_map = label_index_map, data_handler = dh)
