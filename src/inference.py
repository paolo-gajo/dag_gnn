...
dh = DataHandler(format='ud')

## get arguments from the command lind
args = get_args()

args['dir_name'] = './saved_models/model_1_bert-base-uncased_2024-10-26--22:49:38'
...
if 'recipes' in args:
    ...
    assert is_file(config['test_file']), f'{config["test_file"]} is not a valid file, please provide valid conllu file for inference.'
elif not config['test_file']:
    print("No valid input for inference provided, terminating inference.")
    sys.exit(0)
else:
    print("Running inference on the default test file ('data/Parser/test.conllu').")
...
## build dataloader
test_dataloader = build_dataloader(config, loader_type = 'test', collate_function = ner_collate_function, label_index_map=label_index_map)
...
are_trees = dh.is_arborescence(output_file_path)
...
plot_from_conllu(output_file = output_file_path, save_dir = save_dir, data_handler = dh)