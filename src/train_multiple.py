...
from mtrfg.utils import ner_collate_function, train_epoch, run_evaluation
...
def train(config):
    ...
    with open(os.path.join(config['save_dir'], 'log.txt'), 'w') as log_file:
        with contextlib.redirect_stdout(log_file):
            ## build a dataloader
            train_loader = build_dataloader(config, loader_type = 'train', collate_function=ner_collate_function)
            save_json(train_loader.dataset.label_index_map, os.path.join(config['save_dir'], 'labels.json'))

            ## build a validation dataloader
            val_loader = build_dataloader(config, loader_type = 'val', collate_function=ner_collate_function)
